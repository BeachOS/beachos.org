...
BeachOS is Android-based, and the usual procedure to build AOSP applies. If you're already familiar and setup to build Android, you can see the short version to get a quick idea. Otherwise, the long version goes into full detail to set it up from scratch.

This will help you setup a build environment for BeachOS from scratch
...
repo is the tool used to download all the different BeachOS (and AOSP) source code repositories
...
#### BeachOS
Now you can download the BeachOS source code.

```shell
mkdir ~/beachos/android13
...
This will be a big download and may take a long time. Once it's done, you can build BeachOS
...
```shell
./beach/scripts/pixel/device.sh sunfish # Replace sunfish (Pixel 4a) with your device codename here
```

### Building
You made it this far! Now you can finally build BeachOS

```shell
source build/envsetup.sh
lunch beach_sunfish-user # Replace sunfish (Pixel 4a) with your device codename here, and user with userdebug if you want a debug build
...
Congrats! You just compiled and installed a build of BeachOS on your device.
...
## Emulator
To build and run BeachOS Emulator images, see [Emulator](emulator)
