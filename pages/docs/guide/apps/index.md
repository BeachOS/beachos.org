---
title: Apps
description: 📦 App stores, app compatibility, and apps special to BeachOS
toc: true
---
...
* BeachOS gives Aurora Store special privileges so that app updates can be automatically installed. New app installs and uninstalls still must be confirmed as normal.
</div>
</div>
...
The approach that BeachOS takes is to use something called [[microG]], which is a open source replacement for many of the services and libraries provided by Play Services.
...
* **microG enabled, Push notifications enabled**: This is the default for BeachOS. This is identical to the above option, with one minor change - for push notifications, Google services are used semi-anonymously (not tied to a user identity).
...
Check out the [Plexus App Compatibility Database](https://plexus.techlore.tech/). In this database, the label "de-Googled" more or less corresponds to BeachOS with microG disabled, and the label "μ" corresponds to BeachOS with microG enabled.

For more information, see [[microg]].

## Included Apps

BeachOS comes with a list of suggested free software applications to facilitate private online access but the user always has the choice whether to install or not. They'll always be available when needed, even when offline since they're included with the OS.

{% include_relative _app_gallery.html %}

## BeachOS Apps

These applications were developed specifically for use with BeachOS, although some people have also ported these to work with other distributions.
...
<div>
**[[SeedVault]]** allows you to securely backup your phone. All backups are encrypted and decrypted on your device, so you are the only one who is able to read the contents. BeachOS gives SeedVault special permissions to allow full backup and restore.
