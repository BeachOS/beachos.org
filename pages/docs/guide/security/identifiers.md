...
This page attempts to document all the important identifiers, how BeachOS handles each one, and how this differs from stock Android distributions.
...
To read these permanent identifiers, an app requires the `READ_PRIVILEGED_PHONE_STATE` permission, which is not available to a normal app. Since Android 10, only carrier apps and system apps may have this permission. This is true of both BeachOS and stock Android.
...
With BeachOS, there are no included carrier apps and carrier apps cannot be installed automatically.
